...
The exact approved release is public. All six assets plus SHA256SUMS were downloaded without authentication and matched their approved hashes. The downloaded source passed 47 expected isolated command outcomes and 83 tests on Linux x86_64 / CPython 3.12.14 with zero skips. [Retained public-download and execution evidence](https://github.com/thepianistdirector/earth-rehearsal/blob/main/docs/evidence/v05-public-release-verification.json) preserves the full output and its scope. Fresh independent public Mac execution is pending separately from the verified private Mac candidate. Required human/qualified review, external participant evidence and native Tanduna publication remain open.
